...
from typing import Any, Callable, SupportsFloat, TypeAlias, Literal, TypeVar, Type, Final

###############################
# Constants
###############################
frame_rate: Final[int] = 60
window_size: Final[tuple[int, int]] = (1600 // 2, 900 // 2)

_hook_start: Final[str] = 'when_start'
_hook_broadcast: Final[str] = 'when_receive.'
_hook_clone: Final[str] = 'when_clone'
...
class _PygameSpriteWrapper(pygame.sprite.Sprite):
    ...
    def to_py_pos(pos: pygame.Vector2) -> pygame.Vector2:
        return pygame.Vector2(pos.x + window_size[0] / 2, -pos.y + window_size[1] / 2)
...
class ActorMeta(type):
    ...
    def __new__(mcs, name: str, bases: tuple[type, ...], dct: dict[str, Any]) -> type:
        ...
        if new_ty.__name__ in {'Actor', 'Sprite', 'Clone'}:
            def init(_):
                raise NotImplementedError(f'Cannot create an instance of the base sprite type {new_ty.__name__}')
...
class Actor(metaclass=ActorMeta):
    ...
    def is_touching_sprite(self, other: Type['Sprite']) -> bool:
        return self.is_touching(sprite(other))

    def is_touching(self, other: 'Actor') -> bool:
        assert isinstance(other, Actor), f'is_touching expects an Actor, not {other}'
        return pygame.sprite.collide_mask(self._sprite, other.pygame_sprite) is not None
...
class Clone(Actor):
    _forbidden_hooks = [_hook_start]


#########################################
# Internal Representation
#########################################
def _pypurr_proc_while_impl(b: bool) -> bool:
    """
...
def _pypurr_make_proc(f: Callable[[Actor, ...], None]) -> Callable[[Actor, ...], None]:
    """
...
#########################################
class Hooked:
    def __init__(self, f: Callable[[Actor], None], name: str):
        self.func = f
        self.hook = name


def when_start(f: Callable[[Actor], None]) -> Hooked:
    # print(f'Hooking {f} to start')
    return Hooked(_pypurr_make_proc(f), _hook_start)


def when_receive(name: str) -> Callable[[Callable[[Actor], None]], Hooked]:
    def inner(f: Callable[[Actor], None]) -> Hooked:
        # print(f'Hooking {f} to receive')
        return Hooked(_pypurr_make_proc(f), _hook_broadcast + name)
    return inner


def when_clone_start(f: Callable[[Actor], None]) -> Hooked:
    return Hooked(_pypurr_make_proc(f), _hook_clone)


def procedure(f: Callable[[Actor], None]) -> Callable[[Actor], None]:
    return _pypurr_make_proc(f)


def _pypurr_threaded_call(s: Actor, f: Callable[[Actor], None]):
    ...
    n = threading.Thread(target=call)
    n.daemon = True
    n.start()


def _pypurr_threaded_hook(s: Actor, n: str):
    if n in s.hooks:
        for k in s.hooks[n]:
            _pypurr_threaded_call(s, k)
...
def sprite(t: Type[S]) -> S:
    """
    Get the sprite of a given type
    """
    assert issubclass(t, Sprite), 'Type provided to sprite() must be a Sprite type'
    assert t != Sprite, 'Type cannot be the abstract type Sprite'

    return _sprites[t]


def clones(t: Type[C]) -> list[C]:
    """
...
def summon(t: Type[C]) -> C:
    ...
    _pypurr_threaded_hook(c, _hook_clone)
...
def broadcast(n: str):
    ...
    for s in _sprites.values():
        if _hook_broadcast + n in s.hooks:
            _pypurr_threaded_hook(s, _hook_broadcast + n)
...
def run():
    ...
    for s in _sprites.values():
        _pypurr_threaded_hook(s, _hook_start)
